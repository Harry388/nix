{ util, pkgs, ... }@confInps: util.mkModule {
    ...
    programs.zsh = {
        ...
        shellAliases = {
            ...
            gb = "git branch";
            gk = "git checkout";
            gl = "log --graph --abbrev-commit --decorate --format=format:'%C(bold blue)%h%C(reset) - %C(bold green)(%ar)%C(reset) %C(white)%s%C(reset) %C(dim white)- %an%C(reset)%C(auto)%d%C(reset)'";
            gla = "gl --all";
        };
