{ util, pkgs, ... }@confInps: util.mkModule { 
    ...
    home.packages = with pkgs; [
        ...
        (writeShellScriptBin "session" ''
            dir=$(find ~ ~/work ~/programming ~/programming/js ~/programming/zig ~/programming/rust ~/programming/gleam ~/programming/golang ~/programming/elixir -maxdepth 1 -mindepth 1 -type d ! -path "*/.*" | fzf)
            if [[ -n $dir ]] then
